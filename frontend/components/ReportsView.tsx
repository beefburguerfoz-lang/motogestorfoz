          ...
          <tbody className="divide-y">
            {riders.map(rider => (
              <tr key={rider.id} className="hover:bg-slate-50 transition-colors">
                ...
                <td className="px-6 py-4 text-sm text-slate-600">R$ {(((rider.totalEarnings ?? 0) * 0.8).toFixed(2))}</td>
                <td className="px-6 py-4 text-right font-bold text-slate-900">R$ {((rider.totalEarnings ?? 0).toFixed(2))}</td>
              </tr>
            ))}
          </tbody>
        </table>
