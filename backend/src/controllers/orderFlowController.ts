...
export async function createOrderFromSession(arg1: any, arg2?: any, arg3?: any) {
  ...
  if (typeof arg1 === 'string') {
    ...
    // quando vier ConversationSession do Prisma, os dados úteis ficam em session.data
    sessionData = (arg1 && (arg1.data || arg1)) || {};
    companyId = arg2;
    from = arg3;
  }
  ...
  const enderecoRetirada =
    (sessionData.pickup && (sessionData.pickup.formattedAddress || sessionData.pickup.address || sessionData.pickup)) ||
    sessionData.enderecoRetirada ||
    null;
  const enderecoEntrega =
    (sessionData.destination &&
      (sessionData.destination.formattedAddress || sessionData.destination.address || sessionData.destination)) ||
    (sessionData.dropoff && (sessionData.dropoff.formattedAddress || sessionData.dropoff.address || sessionData.dropoff)) ||
    sessionData.enderecoEntrega ||
    null;
  const valor = sessionData.price ?? sessionData.amount ?? sessionData.valor ?? 0;
  const tipoMercadoria = sessionData.cargoType || null;
  const observacao = sessionData.notes || null;
  ...
  try {
    await logAudit({ userId: undefined, action: "ORDER_CREATED_FROM_SESSION", entity: "CORRIDA", entityId: order.id, metadata: JSON.stringify({ from, tipoMercadoria, observacao }) });
  } catch (e) {
